...
-HashMap(ID -> Entity) and HashSet are mostly used for speed and to avoid duplicate entities (especially when adding), ID is not included in hashCode() otherwise those map and set would have issues. However, there are lists that store the ids such that traversing those will get the entities in a desired sequence.
...
-Any error (eg. ID duplication, empty entity, etc.) existing in the CSV file will cause an Exception (all of them were removed)
